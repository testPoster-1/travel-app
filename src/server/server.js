...
const geoUsername = process.env.geo_API_KEY;
...
app.post("/fetchData", async (req, res) => { //see corresponding fetchData POST on main.js
  ...
  try {
    ...
    console.log("Geo data JSON: " + (geoDataJSON.totalResultsCount));

    if (geoDataJSON.totalResultsCount == 0) {
      console.log(`Geocities returned ${geoDataJSON.totalResultsCount} results. No geographical information obtained`);
      let newGeoData = { city: "noCity" }
  ...
  try {
    ...
    console.log("error: no destination data was provided", error);
  }
